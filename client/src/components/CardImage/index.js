...
const CardImage = ({ typeMedia = 'movies', data }) => {
    ...
    return (
        <div className="card-image">
            <img className="card-image__image" src={`https://image.tmdb.org/t/p/w500${data.poster_path}`} alt={data.title} loading='lazy' />
            <div className="card-image__button" role="button" onClick={handleClick}>
                <svg className="card-image__icon" role="presentation" x="0px" y="0px" viewBox="0 0 320.001 320.001">
                ...
                { (typeMedia === 'movies') &&  <span className="card-image__tag card-image__tag--color-movie">Película</span> }
                { (typeMedia === 'series') &&  <span className="card-image__tag card-image__tag--color-serie">Serie</span> }
            </div>
        </div>
    )
}
